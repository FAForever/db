CREATE TABLE `ban` (
  ...
  `expires_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'if null the ban is permanent',
...
SELECT `idUser`, 1, `reason`, FROM_UNIXTIME(`expires_at`), 'GLOBAL' FROM `lobby_ban`;

DROP TABLE `lobby_ban`;

CREATE VIEW `lobby_ban` as 
SELECT ban.player_id, ban.reason, ban.expires_at
FROM ban LEFT JOIN ban_revoke ON ban.id = ban_revoke.ban_id
WHERE level != 'CHAT' 
